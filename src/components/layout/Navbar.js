...
export default function Navbar() {
  ...
  // Desktop link style
  const linkClass = (path) =>
    `font-semibold text-sm tracking-tight pb-1 transition-all duration-300 ${
      pathname === path
        ? "text-[#0050cc] border-b-2 border-[#0050cc]"
        : "text-slate-600 hover:text-[#041627]"
    }`;
  ...
  // Mobile link style
  const mobileLinkClass = (path) =>
    `block text-base font-semibold py-3 transition ${
      pathname === path ? "text-[#0050cc]" : "text-[#0f172a]"
    }`;
    ...
    <>
      ...
      <nav className="fixed top-0 w-full z-50 bg-white/80 backdrop-blur-md shadow-sm">
        <div className="flex justify-between items-center px-6 md:px-8 h-16 max-w-7xl mx-auto">
          ...
          <Link
            href="/"
            className="text-xl font-bold text-[#041627] tracking-tight"
          >
            Luminous Agency
          </Link>
      ...
      >
        ...
        >
          ...
          {/* LINKS */}
          <div className="px-6 py-6 flex flex-col">
            <Link href="/home" onClick={() => setOpen(false)} className={mobileLinkClass("/home")}>Home</Link>
            <Link href="/services" onClick={() => setOpen(false)} className={mobileLinkClass("/services")}>Services</Link>
            <Link href="/portfolio" onClick={() => setOpen(false)} className={mobileLinkClass("/portfolio")}>Portfolio</Link>
            <Link href="/about" onClick={() => setOpen(false)} className={mobileLinkClass("/about")}>About</Link>
            <Link href="/contact" onClick={() => setOpen(false)} className={mobileLinkClass("/contact")}>Contact</Link>

            {/* CTA */}
            <Link
              href="/"
              onClick={() => setOpen(false)}
              className="mt-6 bg-[#0050cc] text-white text-center py-3 rounded-md font-semibold hover:scale-[1.02] active:scale-95 transition"
            >
              Get Started
            </Link>
          </div>
